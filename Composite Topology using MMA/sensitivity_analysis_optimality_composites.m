...
ne = 1;
   ...
   for i = 1:nx
       ...
       dC_drho(ne) = -P * rho(ne)^(P-1) * u' * (A11*KE_A11+A22*KE_A22+A66*KE_A66+A12*KE_A12+A16*KE_A16+A26*KE_A26)* u ;
       dC_dV1A(ne) = -rho(ne)^(P) * u' * (UU2*KE_A11 - UU2*KE_A22)* u ;
       dC_dV2A(ne) = 0;%-rho(ne)^(P) * u' * (UU2/2)*(KE_A16 + KE_A26)* u ;
       dC_dV3A(ne) = -rho(ne)^(P) * u' * (UU3 * KE_A11 + UU3*KE_A22 - UU3*KE_A66 - UU3* KE_A12)* u ;
       dC_dV4A(ne) = 0 ;%-rho(ne)^(P) * u' * (UU3 * KE_A16 - UU3 * KE_A26)* u ;
       ne = ne+1;
   end
   end
   C = F'*U;